...
- Создать **Scheduled Task для автозапуска** — оверлей будет запускаться при входе в Windows автоматически и без UAC-промпта (через `schtasks /RL HIGHEST /SC ONLOGON`)

При установке можно снять галочку «Автозапуск при входе в Windows», если не нужен.

#### ⚠ SmartScreen / Defender при первом запуске
...
> «Защитник Windows заблокировал запуск этого приложения»
...
1. Нажми **«Подробнее»** в окне предупреждения
2. Появится кнопка **«Выполнить в любом случае»** — жми её

**Вариант 2 — заранее разблокировать файл:**
1. ПКМ по скачанному `.exe` → **Свойства**
2. Внизу вкладки «Общие» — галочка **«Разблокировать»** → **ОК**
...
Можешь сверить SHA256 файла с тем, что указан в [Releases](../../releases) — это гарантия что файл не был подменён.
